...
exports.getAllUsers = async (req, res) => {
  try {
    ...
    const [usuariosRows, fields] = await connection.execute(
...
exports.getUser = async (req, res) => {
  try {
    ...
    if (!user.role || !user.role.permisos) {
      return res
        .status(403)
        .json({ error: "El usuario no tiene roles o permisos asignados" });
    }
...
exports.editarPerfil = async (req, res) => {
  ...
  if (!req.usuario || !req.usuario.id) {
    return res
      .status(401)
      .json({ error: "No se ha autenticado correctamente." });
  }
  ...
  try {
    ...
    if (tipo === "empleado") {
      ...
      // Para 'usuario' que puede tener campos en minúsculas
      user.set({
        nombre: nombre,
        apellido: apellido,
        telefono: telefono,
