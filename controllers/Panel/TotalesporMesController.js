...
const insumosMasComprados = async (req, res) => {
    try {
        ...
        const insumosConConteos = insumosMasComprados.map(insumo => {
            const detalle = insumosOrdenados.find(d => d.IdInsumo === insumo.IdInsumos);
            return {
        ...
        res.status(500).json({ error: 'Error al obtener los insumos más comprados' });
    }
};

module.exports = {compararSemana, ventasPorMes, contarEstadosAgenda, agendamientosPorMes, insumosMasComprados};
